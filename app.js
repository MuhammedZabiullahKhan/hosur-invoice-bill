...
function saveGSTSettings() {
    ...
    updateGSTLabel();
    calculateAllTotals();
    const msg = currentLanguage === 'tamil'
...
const translations = {
    tamil: {
        ...
        createdByText: "ஷ்ரீ முஹம்மது ஜபியுல்லா கான் | PrimeSys Solutions",
        ...
        noInvoicesText: "இன்னும் இன்வாய்ஸ் இல்லை",
        ...
        qrUploadTitle: "உங்கள் கட்டண QR",
        qrUploadDesc: "GPay/PhonePe/Paytm QR பதிவேற்றுக",
        ...
        paymentQRInstruction: "GPay, PhonePe, Paytm மூலம் ஸ்கேன் செய்யவும்",
        qrCloseBtn: "மூடு",
        noQRText: "கட்டண QR இல்லை",
        ...
        howToList: "1. உங்கள் கடை பெயர் மற்றும் தொடர்பு விவரங்களை உள்ளிடவும்\n2. வாடிக்கையாளர் பெயர் மற்றும் மொபைல் எண்ணை உள்ளிடவும்\n3. பொருள் பெயரை தட்டச்சு செய்யவும்\n4. அளவு மற்றும் விலையை உள்ளிடவும்\n5. 'இன்வாய்ஸ் சேமி & முன்னோட்டம்' கிளிக் செய்யவும்\n6. PDF-ஐ உங்கள் சாதனத்தில் சேமிக்கவும்\n7. அடுத்த வாடிக்கையாளருக்கு 'புதிய பில்' பயன்படுத்தவும்",
        techTitle: "🛠️ தொழில்நுட்பம்:",
        techText: "உங்கள் இன்வாய்ஸ்கள் IndexedDB-யில் சேமிக்கப்படும். PDF-ஐ எப்போதும் சேமிக்கவும்.",
    ...
    english: {
        ...
        tipText: "💡 Tip: Start typing - auto suggestions appear! Save PDFs to your device!",
        ...
        noInvoicesText: "No invoices yet",
        ...
        qrUploadTitle: "Your Payment QR",
        qrUploadDesc: "Upload GPay/PhonePe/Paytm QR",
        ...
        paymentQRInstruction: "Scan with GPay, PhonePe, or Paytm",
        qrCloseBtn: "Close",
        noQRText: "No payment QR uploaded",
        ...
        howToList: "1. Enter your Business Name and Contact\n2. Add Customer Name and Mobile Number\n3. Type Item Name (auto-suggestions appear)\n4. Enter Quantity and Price\n5. Click 'Save Invoice & Preview'\n6. Save the PDF to your device\n7. Use 'New Bill' for next customer",
        techTitle: "🛠️ Technical:",
        techText: "Your invoices are stored in IndexedDB. Always save PDFs.",
...
function applyTranslations() {
    ...
    const elements = ['appTitle', 'subtitleText', 'createdByText', 'showQRBtnText', 'settingsBtnText', 'installBtnText', 'tipText',
        'businessTitle', 'businessNameLabel', 'businessContactLabel', 'invoicePrefixLabel', 'nextInvoiceLabel', 'itemsLabel',
        'customerNameLabel', 'customerMobileLabel', 'itemNameHeader', 'qtyHeader', 'priceHeader', 'totalHeader', 'subtotalLabel',
        'totalLabel', 'notesLabel', 'historyTitle', 'noInvoicesText', 'filterBtn', 'clearFilterBtn', 'settingsTitle',
        'qrUploadTitle', 'qrUploadDesc', 'uploadQRBtn', 'removeQRBtn', 'gstTitle', 'gstToggleLabel', 'gstPercentLabel',
        'saveGSTBtn', 'themeTitle', 'dataTitle', 'clearDataBtn', 'clearWarning', 'aboutTitle', 'aboutText', 'paymentQRTitle',
        'paymentQRInstruction', 'qrCloseBtn', 'noQRText', 'warningTitle', 'howToTitle', 'techTitle', 'techText'];
    
    elements.forEach(id => { const el = document.getElementById(id); if (el && t[id]) el.textContent = t[id]; });
    ...
    setText('newBillBtnText', t.newBillBtnText);
    setText('addItemBtnText', t.addItemBtnText);
    
    const warningList = document.getElementById('warningList');
    if (warningList && t.warningList) warningList.innerHTML = t.warningList.split('\n').map(item => `<li>${item}</li>`).join('');
    const howToList = document.getElementById('howToList');
    if (howToList && t.howToList) howToList.innerHTML = t.howToList.split('\n').map(item => `<li>${item}</li>`).join('');
    ...
    document.querySelectorAll('.item-name').forEach(input => { input.placeholder = t.itemNamePlaceholder; });
...
function changeTheme(theme) {
    ...
    showToast(translations[currentLanguage].themeChanged || 'Theme changed');
    closeSettings();
}
...
function closePaymentQRModal() { document.getElementById('paymentQRModal').classList.remove('active'); }
...
function handleQRUpload(event) {
    ...
    if (file) {
        ...
        reader.onload = function(e) {
            ...
            showToast(translations[currentLanguage].qrUploadSuccess || 'QR uploaded!');
        };
...
function removeQRCode() {
    ...
    showToast(translations[currentLanguage].qrRemoved || 'QR removed');
}
...
function resetForNewBill() {
    ...
    showToast(t.newBillReady || '✅ Ready for new bill!');
...
function initDB() {
    return new Promise((resolve, reject) => {
        ...
        request.onupgradeneeded = (e) => {
            ...
            if (!db.objectStoreNames.contains('businessNames')) db.createObjectStore('businessNames', { keyPath: 'name' });
            if (!db.objectStoreNames.contains('itemNames')) db.createObjectStore('itemNames', { keyPath: 'name' });
            if (!db.objectStoreNames.contains('customerNames')) db.createObjectStore('customerNames', { keyPath: 'name' });
        };
    });
}
...
function saveToAutoComplete(storeName, name) {
    ...
    return new Promise((resolve, reject) => {
        ...
        getRequest.onsuccess = () => {
            ...
            if (existing) { existing.count = (existing.count || 0) + 1; store.put(existing); }
            else { store.put({ name: cleanName, count: 1, lastUsed: Date.now() }); }
...
async function getNextInvoiceNumber() {
    ...
    const numbers = invoices.filter(inv => inv.invoiceNo && inv.invoiceNo.startsWith(prefix))
        .map(inv => { const num = parseInt(inv.invoiceNo.replace(prefix, '')); return isNaN(num) ? 0 : num; });
        ...
        nameInput.addEventListener('input', async (e) => {
            ...
            if (query && query.length >= 1) {
                ...
                if (suggestions.length > 0 && nameInput.value.trim()) {
                    createAutoComplete(nameInput, suggestions, async (selected) => { await saveToAutoComplete('itemNames', selected); });
                }
            }
        });
        nameInput.addEventListener('blur', async () => {
            if (nameInput.value.trim()) await saveToAutoComplete('itemNames', nameInput.value.trim());
            setTimeout(() => { const dropdown = document.getElementById(`dropdown_${nameInput.id}`); if (dropdown) dropdown.remove(); }, 200);
        });
    }
    update();
}
...
function initializeFirstRow() {
    const tbody = document.getElementById('itemsTable');
    if (tbody) { tbody.innerHTML = ''; itemCounter = 0; addItemRow(); }
}
...
function generateFileName(customerName, date) {
    ...
    if (date && date.includes('/')) { const parts = date.split('/'); formattedDate = `${parts[0]}-${parts[1]}-${parts[2]}`; }
    else { const today = new Date(); formattedDate = `${today.getDate()}-${today.getMonth() + 1}-${today.getFullYear()}`; }
...
function generatePreviewHTML(invoice) {
    ...
    const itemsHtml = invoice.items.map((item, index) => `
        <tr><td style="padding:10px 8px;text-align:center;border:1px solid #ddd;">${index+1}</td>
    ...
    return `<!DOCTYPE html><html><head><meta charset="UTF-8"><title>Invoice ${invoice.invoiceNo}</title><style>*{margin:0;padding:0;box-sizing:border-box}body{font-family:'Inter','Noto Sans Tamil',Arial,sans-serif;background:#f0f2f5;padding:20px;display:flex;justify-content:center}.invoice-container{max-width:900px;width:100%;background:white;border-radius:16px;box-shadow:0 20px 35px -10px rgba(0,0,0,0.15);overflow:hidden}.invoice-header{background:linear-gradient(135deg,#1e3a8a,#3b82f6);color:white;padding:25px 30px;text-align:center}.invoice-header h1{font-size:24px;margin-bottom:5px}.invoice-header p{font-size:12px;opacity:0.9}.invoice-title{background:#f8fafc;padding:12px 30px;border-bottom:2px solid #e2e8f0}.invoice-title h2{color:#1e3a8a;font-size:18px}.filename-info{background:#e8f0fe;padding:10px 30px;font-size:11px;color:#1e3a8a;border-bottom:1px solid #e2e8f0;text-align:center}.customer-info{padding:15px 30px;background:#f8fafc;display:flex;justify-content:space-between;flex-wrap:wrap;gap:15px;border-bottom:1px solid #e2e8f0}.customer-info div{flex:1}.customer-info strong{color:#1f2937;font-size:12px;display:block;margin-bottom:4px}.customer-info p{color:#4b5563;font-size:13px}.items-table{padding:15px 30px}.items-table table{width:100%;border-collapse:collapse}.items-table th{background:#f1f5f9;padding:10px 8px;text-align:left;font-size:12px;font-weight:600;color:#1e293b;border:1px solid #cbd5e1}.items-table td{padding:8px;font-size:12px;color:#334155;border:1px solid #cbd5e1}.totals{padding:15px 30px;background:#f8fafc;text-align:right;border-top:2px solid #e2e8f0}.totals table{width:280px;margin-left:auto;border-collapse:collapse}.totals td{padding:6px 10px;font-size:13px}.totals td:first-child{text-align:left;font-weight:500}.totals td:last-child{text-align:right;font-weight:600}.totals .grand-total td{font-size:16px;font-weight:800;color:#1e3a8a;border-top:2px solid #cbd5e1}.notes{padding:15px 30px;background:white;border-top:1px solid #e2e8f0;font-style:italic;color:#6b7280;font-size:12px}.footer{padding:12px 30px;background:#f1f5f9;text-align:center;font-size:10px;color:#64748b}.button-container{padding:20px 30px;background:white;text-align:center;border-top:1px solid #e2e8f0}.print-btn{background:linear-gradient(135deg,#10b981,#059669);color:white;padding:12px 24px;border:none;border-radius:8px;font-size:16px;font-weight:bold;cursor:pointer;margin-right:10px}.close-btn{background:#6b7280;color:white;padding:12px 24px;border:none;border-radius:8px;font-size:16px;font-weight:bold;cursor:pointer}@media print{.button-container,.print-btn,.close-btn,.filename-info{display:none!important}body{background:white;padding:0}.invoice-container{box-shadow:none;border-radius:0}}</style></head><body>
        <div class="invoice-container"><div class="invoice-header"><h1>${escapeHtml(invoice.businessName)}</h1><p>${escapeHtml(invoice.businessContact)}</p></div>
        ...
        <div class="totals"><tr><td>Subtotal</td><td>₹ ${invoice.subtotal}</td></tr>
        ...
        <div class="button-container"><button class="print-btn" onclick="window.print()">🖨️ ${t.printPDF || 'Print as PDF'}</button><button class="close-btn" onclick="window.close()">❌ ${t.closePreview || 'Close'}</button></div>
        </div><script>document.title = "${fileName.replace('.pdf', '')}";<\/script></body></html>`;
}
...
async function saveInvoice() {
    ...
    if (items.length === 0) { alert(t.noItemAlert || 'Please add at least one item!'); return; }
    ...
    if (editingInvoiceId) {
        ...
        invoice = { ...existing, businessName, businessContact, customerName, customerMobile, items, subtotal: subtotal.toFixed(2), gst: gst.toFixed(2), total: total.toFixed(2), tamilNotes, updatedAt: timestamp };
        await saveInvoiceToDB(invoice);
        showToast(t.updateSuccess || '✅ Invoice updated!');
        editingInvoiceId = null;
        document.getElementById('createInvoiceTitle').innerHTML = t.createInvoiceTitle;
        const saveBtn = document.getElementById('saveInvoiceBtn');
        if (saveBtn) { saveBtn.innerHTML = `<i class="fas fa-save"></i> ${t.saveBtnText}`; saveBtn.classList.remove('bg-orange-500', 'hover:bg-orange-600'); saveBtn.classList.add('btn-primary'); }
        const cancelBtn = document.getElementById('cancelEditBtn');
        if (cancelBtn) cancelBtn.style.display = 'none';
    } else {
        const invoiceNo = await getNextInvoiceNumber();
        invoice = { id: Date.now(), invoiceNo, date, businessName, businessContact, customerName, customerMobile, items, subtotal: subtotal.toFixed(2), gst: gst.toFixed(2), total: total.toFixed(2), tamilNotes, footprint: getDigitalFootprint(), timestamp };
        await saveInvoiceToDB(invoice);
        document.getElementById('customerName').value = '';
        document.getElementById('customerMobile').value = '';
        showToast(t.saveSuccess || '✅ Invoice saved!');
    }
...
async function editInvoice(invoiceId) {
    ...
    invoice.items.forEach(item => {
        ...
        const newRow = document.createElement('tr');
        newRow.id = `itemRow_${rowId}`;
        newRow.innerHTML = `<td class="border p-1"><input type="text" class="item-name w-full p-2 border rounded-lg" value="${escapeHtml(item.name)}" style="font-size:16px"></td>
    ...
    const saveBtn = document.getElementById('saveInvoiceBtn');
    if (saveBtn) { saveBtn.innerHTML = `<i class="fas fa-edit"></i> ${t.updateBtnText}`; saveBtn.classList.remove('btn-primary'); saveBtn.classList.add('bg-orange-500', 'hover:bg-orange-600'); }
    ...
    } else { cancelBtn.style.display = 'flex'; const span = cancelBtn.querySelector('span'); if (span) span.textContent = t.cancelEditText; }
    window.scrollTo({ top: 0, behavior: 'smooth' });
...
async function deleteInvoice(invoiceId) {
    const t = translations[currentLanguage];
    if (confirm(t.confirmDelete || 'Delete this invoice?')) {
        await deleteInvoiceFromDB(invoiceId);
        await loadInvoices();
        showToast(t.deleteSuccess || '✅ Invoice deleted!');
...
function filterByDate() { currentFilterDate = document.getElementById('filterDate').value; loadInvoices(); }
function clearFilter() { document.getElementById('filterDate').value = ''; currentFilterDate = null; loadInvoices(); }
...
async function loadInvoices() {
    ...
    const t = translations[currentLanguage];
    if (currentFilterDate) invoices = invoices.filter(inv => inv.date === currentFilterDate);
    ...
    container.innerHTML = sorted.map(inv => `
        <div class="invoice-item border rounded-xl p-3 hover:shadow-md">
            ...
            <div class="flex gap-2 mt-2 pt-2 border-t">
                <button onclick="viewInvoicePreview(${inv.id})" class="flex-1 bg-blue-500 text-white px-2 py-1.5 rounded-lg text-xs"><i class="fas fa-eye"></i> ${t.viewText}</button>
                <button onclick="editInvoice(${inv.id})" class="flex-1 bg-yellow-500 text-white px-2 py-1.5 rounded-lg text-xs"><i class="fas fa-edit"></i> ${t.editText}</button>
                <button onclick="deleteInvoice(${inv.id})" class="flex-1 bg-red-500 text-white px-2 py-1.5 rounded-lg text-xs"><i class="fas fa-trash"></i> ${t.deleteText}</button>
            </div>
        </div>
    `).join('');
}

async function viewInvoicePreview(invoiceId) { const invoice = await getInvoiceById(invoiceId); if (invoice) showPreview(invoice); }

async function clearAllData() {
    const t = translations[currentLanguage];
    if (confirm(t.clearConfirm || 'Delete all invoices?')) {
...
async function setupBusinessAutoComplete() {
    ...
    input.addEventListener('input', async (e) => {
        ...
        if (query.length >= 1) {
            ...
            createAutoComplete(input, suggestions, async (selected) => { await saveToAutoComplete('businessNames', selected); });
        }
    });
    input.addEventListener('blur', async () => { if (input.value.trim()) await saveToAutoComplete('businessNames', input.value.trim()); });
}
...
async function setupCustomerAutoComplete() {
    ...
    input.addEventListener('input', async (e) => {
        ...
        if (query.length >= 1) {
            ...
            createAutoComplete(input, suggestions, async (selected) => { await saveToAutoComplete('customerNames', selected); });
        }
    });
    input.addEventListener('blur', async () => { if (input.value.trim()) await saveToAutoComplete('customerNames', input.value.trim()); });
}
